...
func InitSchemaRegistry() error {
	...
	if schemaRegistryURL == "" {
		...
		flightlog.LogWarn("SCHEMA_REGISTRY_URL not set, using default: %s", schemaRegistryURL)
	}
	...
	if err != nil {
		flightlog.LogError("Failed to get schema from registry: %v", err)
		return err
	}
	flightSchema = schema
	flightlog.LogInfo("Schema loaded successfully, ID: %d", schema.ID())
	return nil
}
...
func ValidateFlightData(data []byte) error {
	...
	for _, flight := range flights {
		if err := validateFlightRecord(flight); err != nil {
			flightlog.LogWarn("Flight record validation failed: %v", err)
			return err
		}
	}
	
	flightlog.LogDebug("Validated %d flight records", len(flights))
	return nil
}
...
func validateFlightRecord(flight map[string]interface{}) error {
	...
	for _, field := range requiredFields {
		if _, exists := flight[field]; !exists {
			flightlog.LogError("Missing required field: %s", field)
